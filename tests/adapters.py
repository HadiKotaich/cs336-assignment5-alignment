...
def run_tokenize_prompt_and_output(
    ...
    for prompt_str, output_str in zip(prompt_strs, output_strs):
        ...
        labels.append(full)
        response_masks.append([0] * len(prompt_ids) + [1] * len(output_ids))

    max_prompt_and_output_len = max(len(ids) for ids in input_ids)
    input_ids_tensors = torch.tensor(
        [
            (ids + [pad_token] * (max_prompt_and_output_len - len(ids)))[:-1]
    ...
    response_masks_tensors = torch.tensor(
        [
            (mask + [0] * (max_prompt_and_output_len - len(mask)))[1:]
    ...
    return {
        ...
        "response_mask": response_masks_tensors,
    }
...
def run_compute_entropy(logits: torch.Tensor) -> torch.Tensor:
    ...
    # H = -sum (p * log p)
    p_logp = p * log_p  # (B, T, V)
    entropies = -p_logp.sum(dim=-1)  # (B, T)
    return entropies
...
def run_sft_microbatch_train_step(
    ...
    loss = run_masked_normalize(
        policy_log_probs, response_mask, normalize_constant=normalize_constant
    )
...
def run_grpo_microbatch_train_step(
    ...
    loss = run_masked_mean(loss, response_mask, dim=1).mean()
